...
static void *
get_next_image (char *img_buf, int idx, float *dst_img)
{
    int x, y;
    int w = 28;
    int h = 28;
...
int 
infer_float (tflite::Interpreter *interpreter)
{
    int num_imgs = 0;
    ...
    float *in_tensor_ptr  = interpreter->typed_input_tensor<float>(0);
    float *out_tensor_ptr = interpreter->typed_output_tensor<float>(0);
...
#if defined (VERBOSE)
        for (int i = 0; i < 10; i ++)
            fprintf (stderr, "%8.4f ", out_tensor_ptr[i]);
    ...
    fprintf (stderr, "\n");
    fprintf (stderr, "----------------------------------------\n");
    fprintf (stderr, "  accuracy(float) = %f [%d/%d]\n",  accuracy, num_correct, num_imgs);
    fprintf (stderr, "----------------------------------------\n");
