...
#
# ■TensorFlow の Frozen PB を、量子化TFLiteに変換する
#
#
# [usage] 
#   > ./convert_pb_to_tflite_quant frozen_pb in_node out_node
#
# ./convert_pb_to_tflite_quant.sh mobilenet_v1_1.0_224_frozen.pb input MobilenetV1/Predictions/Reshape_1
...
if [ $# -ne 4 ]
then
  echo "[usage] convert_pb_to_tflite_quant frozen_pb in_node out_node"
  exit 0
fi
...
TFLITE_FILE=${PB_FILE}.tf_quant.tflite

#
#  入力テンソルの量子化パラメータ
#
# +------------------------------------+------------------+------+------+
# |                                    |入力テンソル変換式| mean |stddev|
# +------------------------------------+------------------+------+------+
# |  0.0～1.0 を0～255に量子化した場合 | (IN -   0) / 255 |    0 |  255 |
# | -1.0～1.0 を0～255に量子化した場合 | (IN - 128) / 127 |  128 |  127 |
# +------------------------------------+------------------+------+------+
#
MEAN_VAL=0
STD_DEV=255
