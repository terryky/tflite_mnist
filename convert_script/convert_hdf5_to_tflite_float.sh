...
if [ $# -ne 4 ]
then
  echo "[usage] convert_hdf5_to_tflite_float hdf5_file_path in_node out_node innode_shape"
  exit 0
fi
...
TFLITE_FILE=hdf5_model.tf_float.tflite

tflite_convert \
  --keras_model_file=${HDF5_PATH} \
